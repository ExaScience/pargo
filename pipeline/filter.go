...
// A Filter is a function that returns a Receiver and a Finalizer to
// be added to a node. It receives a pipeline, the kind of node it
// will be added to, and the expected total data size that the
// receiver will be asked to process.
//
...
// to determine. The dataSize parameter is a pointer whose contents
// can be modified by the filter, for example if this filter increases
// or decreases the total size for subsequent filters, or if this
// filter can change dataSize from an unknown to a known value, or
// vice versa, must change it from a known to an unknown value.
//
