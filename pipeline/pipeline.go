...
// pipelines.
//
// A Pipeline feeds batches of data through several functions that can
// be specified to be executed in encounter order, in arbitrary
// sequential order, or in parallel.  Ordered, sequential, or parallel
// stages can arbitrarily alternate.
//
// A Pipeline consists of a Source object, and several Node objects.
//
// Source objects that are supported by this implementation are
// arrays, slices, strings, channels, and bufio.Scanner objects, but
// other kinds of Source objects can be added by user programs.
//
// Node objects can be specified to receive batches from the input
// source either sequentially in encounter order, which is always the
// same order in which they were originally encountered at the source;
// sequentially, but in arbitrary order; or in parallel. Ordered nodes
// always receive batches in encounter order even if they are preceded
// by arbitrary sequential, or even parallel nodes.
//
// Node objects consist of filters, which are pairs of receiver and
// finalizer functions. Each batch is passed to each receiver
// function, which can transform and modify the batch for the next
// receiver function in the pipeline. Each finalizer function is
// called once when all batches have been passed through all receiver
// functions.
//
// Pipelines do not have an explicit representation for
// sinks. Instead, filters can use side effects to generate results.
//
...
// implemented in this package, so that user programs are typically
// not concerned with Node methods at all.
type Node interface {
...
//
// If source does not implement the Source interface, the pipeline
// uses reflection to create a proper source for arrays, slices,
// strings, or channels.
//
// It is safe to call Source multiple times before Run or
// RunWithContext is called, in which case only the last call to
// Source is effective.
...
// the data source for this pipeline, if the expected total size for
// this pipeline's data source is known or can be determined easily.
//
...
//
// It expects a context and a cancel function as parameters, for
// example from context.WithCancel(context.Background()). It does not
// ensure that the cancel function is called at least once, so this
// must be ensured by the function calling RunWithContext.
//
// RunWithContext should only be called after a data source has been
// set using the Source method, and one or more Node objects have been
// added to the pipeline using the Add method. NofBatches can be
// called before RunWithContext to deal with load imbalance, but this
// is not necessary since RunWithContext chooses a reasonable default
// value.
//
// RunWithContext prepares the data source, tells each node that
// batches are going to be sent to them by calling Begin, and then
// fetches batches from the data source and sends them to the
// nodes. Once the data source is depleted, the nodes are informed
// that the end of the data source has been reached.
...
// RunWithContext(context.WithCancel(context.Background())), and
// ensures that the cancel function is called at least once when the
// pipeline is done.
//
// Run should only be called after a data source has been set using
// the Source method, and one or more Node objects have been added to
// the pipeline using the Add method. NofBatches can be called before
// Run to deal with load imbalance, but this is not necessary since
// Run chooses a reasonable default value.
//
// Run prepares the data source, tells each node that batches are
// going to be sent to them by calling Begin, and then fetches batches
// from the data source and sends them to the nodes. Once the data
// source is depleted, the nodes are informed that the end of the data
...
// FeedForward must be called in the Feed method of a node to forward
// a potentially modified data batch to the next node in the current
// pipeline.
//
// FeedForward is used in Node implementations. User programs
// typically do not call FeedForward.
//
// FeedForward must be called with the pipeline received as a
// parameter by Feed, and must pass the same index and seqNo received
// by Feed. The data parameter can be either a modified or an
// unmodified data batch. FeedForward must always be called, even if
// the data batch is unmodified, and even if the data batch is or
// becomes empty.
